...
export const Chat: React.FC<ChatProps> = ({ messages, input, handleInputChange, onSubmit, isLoading, messagesEndRef, onBackToEntrance, caseId }) => {
  ...
  return (
    <Card className="w-full max-w-4xl shadow-lg">
      ...
      <CardContent className="h-[60vh] overflow-y-auto p-4">
        {caseData && caseData.logs && caseData.logs.length > 0 ? (
          caseData.logs.map((log: any, idx: number) => (
            <div key={idx} className={`mb-4 flex ${log.role === "user" ? "justify-end" : "justify-start"}`}>
              <div
