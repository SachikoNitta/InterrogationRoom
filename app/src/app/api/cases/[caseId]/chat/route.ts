...
// POST /api/cases/[caseId]/chat - 容疑者とのチャット
...
容疑者情報: ${JSON.stringify(summaryData?.suspectInfo || [])}

重要な指示:
- 最初は犯行を否認しています。
- 自分の犯行を隠すための嘘をついてください。
- 嘘を暴く証拠が突きつけられた場合は、感情的な反応（焦り・怒り・戸惑いなど）をしてください。
- プレイヤーの追及が鋭く、かつ状況証拠が揃った場合は、しぶしぶ自白してください。
- 一回の返答は200文字程度に収めてください。`,
    });
    
    // チャット履歴を取得
    const existingLogs = caseData.logs || [];
    ...
    const chat = model.startChat({
      history: chatHistory.slice(0, -1), // 最後のメッセージは除く
    });
      ...
      async start(controller) {
        try {
          ...
          await db.collection('cases').doc(caseId).update({
            logs: [...existingLogs, logEntry, responseEntry],
            updatedAt: new Date()
          });
