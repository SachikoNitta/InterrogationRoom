...
def create_case(token: dict = Depends(auth_service.verify_id_token)) -> case_dto.Case:
...
@router.get("/api/cases/{caseId}", response_model=case_dto.Case)
...
@router.get("/api/cases", response_model=List[case_dto.Case])
def get_cases(token: dict = Depends(auth_service.verify_id_token)) -> List[case_dto.Case]:
